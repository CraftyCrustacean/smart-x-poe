...
function DeviceList() {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ['devices'],
        queryFn: () => fetch('http://localhost:8080/api/devices').then(res => res.json()),
    });

    const { data: recentBatches } = useQuery({
        queryKey: ['recentBatches'],
        queryFn: () => fetch('http://localhost:8080/api/telemetry/recent').then(res => res.json()),
    });
    ...
    const onlineDeviceIds = useMemo(() => {
        ...
        const latestBatch = recentBatches.reduce((max, current) => {
            const currentTimestamp = new Date(current.timestamp || current.item1).getTime();
        ...
        const deviceIds = packets.map(packet => packet.deviceId || packet.device_id);
    ...
    const subzones = [...new Set(zoneDevices.map(d => d.topology?.subzone).filter(Boolean))];
    ...
    if (!selectedZone) {
        content = (
        <div>
            <h2 className="text-xl font-semibold mb-4 text-slate-800">Select a Zone</h2>
            ...
            {zoneCounts.map(({ zone, subzoneCount }) => (
                ...
                >
                <span className="font-medium text-slate-900">{zone_labels[zone] || zone}</span>
                <span className="text-sm text-slate-500">{subzoneCount} Subzones</span>
                </button>
            ))}
            </div>
        </div>
        ...
        const filteredDevices = data.filter(
            d => d.topology?.zone === selectedZone && d.topology?.subzone === selectedSubzone
        );
        ...
        content = (
            <div>
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-semibold text-slate-800">
                        Devices in {zone_labels[selectedZone] || selectedZone} &gt; {selectedSubzone}
                    </h2>
                    <button onClick={() => setSelectedSubzone(null)} 
                    className="px-3 py-1.5 rounded-md bg-orange-300 text-sm text-slate-900 hover:bg-orange-400 transition-colors">
                        Back
                    </button>
                    </div>
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                    <div className="grid grid-cols-[auto_1fr_1fr_1fr] gap-3 px-4 py-2 bg-slate-50 text-xs font-bold text-slate-500 uppercase tracking-wide border-b border-slate-200">
                        <span></span>
                        <span>Device ID</span>
                        <span>Chainage</span>
                        <span>Category</span>
                        ...
                        <div className="divide-y divide-slate-200">
                            ...
                            return (
                                ...
                                <span
                                    ...
                                    style={{ backgroundColor: isOnline ? '#22c55e' : '#e44040' }}
                                    title={isOnline ? "Active in latest telemetry" : "Offline"}
                                />
                                <span className="text-slate-900">{device.device_id}</span>
                                <span className="text-slate-500">{device.location?.chainage_km} km</span>
